...
static bool shd_preset_value(const QSettings *set, const QString &key, QString &value);
static bool shd_preset_rd_file(QIODevice &device, QSettings::SettingsMap &map);

BYTE shd_preset_parse(const uTCHAR *file) {
	static const QSettings::Format cfg = QSettings::registerFormat("shd", shd_preset_rd_file, nullptr);
	QSettings *set;
	...
	if (QString::compare(fi.suffix(), "cgp", Qt::CaseInsensitive) == 0) {
		// the ".cgp" format points to shaders written in Cg, whose runtime has
		// been removed: the modern equivalent is the ".glslp" preset.
		log_error(uL("shader;the Cg preset format is no longer supported, please use a '.glslp' preset"));
		return (EXIT_ERROR);
	}
	if (QString::compare(fi.suffix(), "glslp", Qt::CaseInsensitive) != 0) {
		return (EXIT_ERROR);
	}
	...
	for (i = 0; i < se.pass; i++) {
		...
		if (!shd_preset_value(set, key, value)) {
			value.replace('\\', '/');
		...
		if (!shd_preset_value(set, key, value)) {
			if (QString::compare(value, "clamp_to_border", Qt::CaseInsensitive) == 0) {
				...
				log_warning(uL("shader;invalid " uPc("") " attribute, using default value"), qPrintable(key));
				sp->wrap = TEXTURE_WRAP_BORDER;
			}
		}
		...
		// scale
		{
			...
			shd_preset_value(set, sc_type, value);
			sc_type = value;

			sc_type_x = QString("scale_type_x%1").arg(i);
			shd_preset_value(set, sc_type_x, value);
			sc_type_x = value;

			sc_type_y = QString("scale_type_y%1").arg(i);
			shd_preset_value(set, sc_type_y, value);
			sc_type_y = value;
			...
			if (!sc_type.isEmpty() || !sc_type_x.isEmpty() || !sc_type_y.isEmpty()) {
				...
				if (sc_type_x.length() > 0) {
					if (QString::compare(sc_type_x, "source", Qt::CaseInsensitive) == 0) {
						...
						log_warning(uL("shader;invalid scale_type_x attribute"));
						delete (set);
						return (EXIT_ERROR);
					}
				}
				...
				if (sc_type_y.length() > 0) {
					if (QString::compare(sc_type_y, "source", Qt::CaseInsensitive) == 0) {
						...
						log_warning(uL("shader;invalid scale_type_y attribute"));
						delete (set);
						return (EXIT_ERROR);
					}
				}
	...
	// textures
	if (!shd_preset_value(set, "textures", value)) {
		list = value.split(';');
		...
		foreach (const QString &ele, list) {
			...
			// path
			if (!shd_preset_value(set, ele, value)) {
				value.replace('\\', '/');
	...
	// parameters
	if (!shd_preset_value(set, "parameters", value)) {
		list = value.split(';');
...
BYTE shd_pragma_param(const char *code, const uTCHAR *path) {
	...
	if (path && path[0]) {
		if (!file.open(QIODevice::ReadOnly)) {
			log_error(uL("shader;can't open file '" uPs("") "'"), path);
			return (EXIT_ERROR);
		}
	...
	if (params) {
		...
		for (i = 0; i < params; i++) {
			...
			log_info_box(uL("%d;" uPc("") " = %f"), i, ps->name, ps->value);
		}
	}
